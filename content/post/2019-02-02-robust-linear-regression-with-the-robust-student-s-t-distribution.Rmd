...
Regression models typically use the Gaussian likelihood. Say you have some criterion variable $y$, which you can describe with a mean $\mu$ and standard deviation $\sigma$. Further, you'd like to describe $y$ using a predictor $x$. Using the Gaussian likelihood, we can describe the model as
...
The models differ by their intercepts, slopes, sigmas, and $\nu$s. For the sake of this project, we'll focus on the slopes. Here we compare the different Bayesian models' slopes by their posterior means and 95% intervals in a coefficient plot.
