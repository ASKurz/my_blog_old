---
title: Bayesian meta-analysis in brms, Rough draft
...
Before we proceed, I should acknowledge that this section is heavily influenced by Matti Vourre’s great blog post, [*Meta-analysis is a special case of Bayesian multilevel modeling*](https://vuorre.netlify.com/post/2016/2016-09-29-bayesian-meta-analysis/). Since McElreath’s text doesn’t directly address meta-analyses, we’ll also have to borrow a bit from Gelman, Carlin, Stern, Dunson, Vehtari, and Rubin’s [*Bayesian data analysis, Third edition*](http://www.stat.columbia.edu/~gelman/book/). Here, Gelman and colleagues introduce the topic:
...
where $y_i$ = the point estimate for the effect size of a single study, which is presumed to have been a draw from a Normal distribution centered on $\theta_i$. The standard error from study $i$ is specified $\sigma_i$, which is also a stand-in for the standard deviation of the Normal distribution from which the point estimate was drawn.

But this has all been about study $i$. We further presume that study $i$ is itself just one draw from a population of studies, each of which have their own effect sizes. As such. we presume $\theta_i$ itself has a distribution following the form

$$\theta_i \sim \text{Normal} (\mu, \tau)$$

where $\mu$ is the meta-analytic effect (i.e., the population mean) and $\tau$ is the variation around that mean. 

Since there’s no example of a meta-analysis in the text, we’ll have to get our data elsewhere. Our data `spank` were transcribed directly from Gershoff and Grogan-Kaylor’s (2016) paper, [*Spanking and Child Outcomes: Old Controversies and New Meta-Analyses*](https://pdfs.semanticscholar.org/0d03/a2e9f085f0a268b4c0a52f5ac31c17a3e5f3.pdf). You can find it in [this project’s GitHub repository](https://github.com/ASKurz/Statistical_Rethinking_with_brms_ggplot2_and_the_tidyverse). 
...
The last two lines, of course, spell out our priors. In psychology, it’s pretty rare to see an *Cohen’s d* greater than the absolute value of $\pm 1$. So in the absence of more specific domain knowledge, it seems like $\text{Normal} (0, 1)$ is a reasonable place to start. Just like McElreath used $\text{HalfCauchy} (0, 1)$ as the default prior for the group-level standard deviations, it makes sense to use it here for our meta-analytic $\tau$ parameter.
...
One thing you might notice is our `se(se)` function excluded the `sigma` argument. If you recall from section 14.1, we specified `sigma = T` in our measurement-error models. The brms default is that within `se()`, `sigma = FALSE`. As such, we have no estimate for sigma the way we would if we were doing this analysis with the raw data from the studies. But if you are fortunate enough to have the raw data from all the studies in your meta-analysis, by all means, set `sigma = TRUE` and perhaps consider fitting a [distributional model](https://cran.r-project.org/web/packages/brms/vignettes/brms_distreg.html) in which the study-level standard deviations are allowed to vary or are perhaps even partially pooled toward a grand mean.
...
There are a few things to note. First, all of our outcomes are negative. We prefer conditions associated with lower values. Second, the way the data are coded, treatment = spanking and comparison = non-spanking. So larger effect sizes are interpreted as more negative outcomes associated with children having been spanked. That is, it appears spanking children is associated with worse outcomes. What might not be immediately apparent is that even though there are 111 cases in the data, there are only 76 distinct studies.
...
So at this point, the big story is there's more variability between the studies than there is the outcomes. But I still want to get a sense of the individual outcomes. Here we'll use `tidybayes::geom_halfeyeh()` to help us make our version of a forest plot.
...
Let's go one step further with the model. Doubling back to Gelman and colleagues, we read:
...
When I use dummies in my models, I generally prefer to have the majority group stand as the reference category. As such, I typically name those variables by the minority group. In this case, most occasions are based on within-participant designs. Thus, we’ll go ahead and add the `between` variable to the model.

```{r b14.7, cache = T, message = F, warning = F, results = 'hide'}
b14.7 <- 
  brm(data = spank, family = gaussian,
      d | se(se) ~ 1 + between + (1 | study) + (1 | outcome),
      prior = c(prior(normal(0, 1), class = Intercept),
...
There are other things you might do with these data. For examply, you might check for trends by year or, as the authors did in their manuscript, distinguish among different severities of corporal punishment. But I think this is as far as I’d like to go with these data. 
