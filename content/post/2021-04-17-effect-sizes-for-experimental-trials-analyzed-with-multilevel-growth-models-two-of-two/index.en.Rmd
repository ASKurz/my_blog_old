...
Though participants tend to increase in both groups, those in the treatment condition appear to have increased at a more rapid pace. We want an standardized effect sizes that can capture those differences. We'll begin to explain what that will be, next.

## Model

### We need a framework.

Traditional analytic strategies, such as ordinary least squares (OLS) regression and the analysis of variance (ANOVA) framework, can work okay with data collected on one or two time points. In his [-@feingoldEffectSizeForGMA2009; -@feingoldARegressionFramework2013] work, which is the inspiration for this blog series, Feingold recommended what he called growth-modeling analysis (GMA) for data collected on $3+$. If you're not familiar with the term, this is a longitudinal version of what others have called hierarchical linear models, mixed-effects models, random-effects models, or what I prefer to call *multilevel models*. For longitudinal data, I'm fond of the term *multilevel growth model*. If you're interested, Raudenbush and Bryk touched on the historic origins of several of these terms in their [-@raudenbushHLM2002] text (Chapter 1).
...
For this particular data set, the value Feingold used is the same for either of the experimental conditions at baseline.
