...
where $p_i$ is the probability of player $i$, $\alpha_\text{player}$ is a vector of $\text{player}$-specific intercepts from within the logistic regression model, and each of those intercepts are given a $\text{Normal} (0. 1.5)$ prior on the log-odds scale. (If this is all new and confusing, spend some time reviewing Bayesian logistic regression. I'll recommended some sources at the end.)
...
Also, check out the provocative preprint by Davis-Stober, Dana and Rouder, [*When are sample means meaningful? The role of modern estimation in psychological science*](https://osf.io/2ukxj/).
